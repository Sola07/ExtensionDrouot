...
export default function App() {
  ...
  async function loadItems(filter = ITEM_SCOPE) {
    setLoading(true);
  ...
  async function handleStateChange(lotId, newState) {
    try {
      ...
      await loadItems(ITEM_SCOPE);
    } catch (error) {
      console.error('Error updating state:', error);
    }
  }

  async function handleClearLots() {
    try {
      const response = await chrome.runtime.sendMessage({
      ...
      if (response.success) {
        setItems([]);
        console.log('[Drouot Monitor] Toutes les ventes ont été effacées');
      }
    } catch (error) {
    ...
    <div className="app">
      <header className="app-header">
        <div className="app-title">
          <img src={logoUrl} alt="Drouot Monitor" className="app-logo" />
          <div>
            <div className="app-title-label">Drouot</div>
          </div>
        </div>
      ...
      <div className="app-content">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <h2>Aucun lot</h2>
            <p>Visitez Drouot.com pour découvrir de nouveaux lots</p>
          </div>
        ) : (
          <ItemList
            items={items}
